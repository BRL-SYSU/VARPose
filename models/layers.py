...
class VectorQuantizer(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, commitment_cost, use_ema=True, decay=0.99):
        ...
        # Use nn.Embedding, easy to understand and required under gradient-update mode
        ...
        if self._use_ema:
            # register_buffer ensures these tensors move with the model (cpu/gpu) and are not treated as model parameters
            self.register_buffer("ema_cluster_size", torch.zeros(num_embeddings))
            # ema_dw accumulates encoder output vectors matched to each codebook entry
            self.register_buffer("ema_dw", self.codebook.weight.data.clone())

        # --- State required for codebook reset (as buffers) ---
    ...
    def forward(self, inputs):
        ...
        # --- Loss computation ---
        if self._use_ema:
            # When using EMA, the VQ loss only includes commitment_loss
            commitment_loss = F.mse_loss(inputs, quantized.detach())
            vq_loss = self._commitment_cost * commitment_loss
        else:
            # When using gradient updates, it includes two parts
...
class MlpMixerBlock(nn.Module):
    """
    An MLP-Mixer block for mixing information across the token and channel dimensions.
    """
    ...
    def forward(self, x):
        # x shape: (B, N, D), N is the number of tokens
        # Token mixing
...
class PositionEmbedding(nn.Module):
    def __init__(self, seq_dim, embed_dim):
        super(PositionEmbedding, self).__init__()  # Add this line
        ...
        position = torch.arange(self.seq_dim, dtype=torch.float32).unsqueeze(1)  # (input_joints, 1)
         # Fix: should compute 1 / (10000 ** (2 * j / embed_dim))
...
class TokenExpansion(nn.Module):
    def __init__(self, tokens_per_joint=6, strategy='balanced'):
        super().__init__()
    ...
    def forward(self, x:torch.Tensor, adj_matrix:torch.Tensor|None):  # x: (B, J, D), adj_matrix: (J, J)
        B, J, D = x.shape
        if self.tokens_per_joint ==1:
            return x
        
        if self.strategy == 'balanced':
            if self.tokens_per_joint % 2 != 0:
                raise ValueError("Balanced strategy requires an even number of tokens per joint.")
        ...
        elif self.strategy == 'position_only': # <--- new elif branch
            tokens = x.unsqueeze(2).expand(-1, -1, self.tokens_per_joint, -1)
...
class TokenAggregation(nn.Module):
    ...
    def forward(self, x:torch.Tensor):  # (B, 6J, D)
        ...
        if self.strategy == 'balanced':
            ...
            # Process position-information tokens and adjacency-information tokens separately
            ...
            out = pos + adj
        elif self.strategy == 'position_only':
            out = x.mean(dim=2)
        else:
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, offset: int = 0):
        ...
        self.seq_len_cached = seq_len if offset == 0 else None # Only cache in standard mode
        # Compute the correct position range based on offset and seq_len
