...
class FusionPose(nn.Module):
    def __init__(self, n_joints, hidden_dim, cond_dim, num_heads=8):
        super().__init__()
        self.n_joints = n_joints
        self.hidden_dim = hidden_dim

        self.sparse_embed = nn.Linear(cond_dim, hidden_dim)
        self.dense_embed = nn.Linear(cond_dim, hidden_dim)
        self.sparse_norm = nn.LayerNorm(hidden_dim)
        ...
        self.refine_proj = nn.Sequential(
            nn.Linear(n_joints * hidden_dim, hidden_dim),
            nn.GELU(),
    ...
    def forward(self, sparse_2d:torch.Tensor, dense_2d:torch.Tensor, sparse_feat:torch.Tensor):
        query = self.sparse_embed(sparse_2d); key_and_value = self.dense_embed(dense_2d)
        query = self.sparse_norm(query); key_and_value = self.dense_norm(key_and_value)
        attn_out, _ = self.cross_attn(query, key_and_value, key_and_value)
        refine = query + attn_out
        refine = refine + self.cross_mlp(self.cross_norm(refine))

        refine = refine.reshape(refine.shape[0], -1)
        refine = self.refine_proj(refine)
        refine = self.align_mlp(torch.cat([refine, sparse_feat], dim=-1))
        return refine[:, :self.hidden_dim]
...
class ScoreModelFC_Adv(nn.Module):
    ...
    def __init__(self, config,
        ...
        self.num_dense_joints = config.model.num_dense_joints
        if self.num_dense_joints > 0:
            self.fusion_pose = FusionPose(self.n_joints, hidden_dim, cond_dim)
    ...
    def forward(self, batch, t, condition, mask):
        ...
        if condition.shape[-2]>17:
            if self.num_dense_joints>0:
                dense_2d = condition[:, 17:17+self.num_dense_joints, :]
            condition = condition[:, :17, :]
        elif self.num_dense_joints>0:
            raise RuntimeError("num_dense_joints must be 0 when using 2D sparse pose")
        ...
        if self.num_dense_joints > 0:
            cond = self.fusion_pose(condition.view(bs, self.n_joints, 3), dense_2d, cond)
