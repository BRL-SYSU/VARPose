...
kps_left = kps_left + kps_dense_left
kps_right = kps_right + kps_dense_right
...
data_train = np.load("data/data_train_3dhp_dense_annot3.npz", allow_pickle=True)['data'].item()
for seq in sorted(data_train.keys()):
    for cam in data_train[seq][0].keys():
        ...
        data_2d[..., pseudo_joints_idx, :] = data_2d[..., 14:15, :]

        data_2d[..., :2] = normalize_screen_coordinates(data_2d[..., :2], w=2048, h=2048)
        out_poses_2d_train[(subject_name, seq_name, cam)] = data_2d


data_test = np.load("data/data_test_3dhp_dense_annot3.npz", allow_pickle=True)['data'].item()
for seq in sorted(data_test.keys()):

    anim = data_test[seq]

    total_valid_frame_test[seq] = anim["valid"]
...
num_joints = 17
num_joints_dense = 144 if not args.only_17 else 0

model_pos_train = D3DP(args, joints_left, joints_right, is_train=True, num_joints=num_joints, num_joints_dense=num_joints_dense, dense_fuse_strategy=args.dense_fuse_strategy)
model_pos_test_temp = D3DP(args,joints_left, joints_right, is_train=False, num_joints=num_joints, num_joints_dense=num_joints_dense, dense_fuse_strategy=args.dense_fuse_strategy)
model_pos = D3DP(args,joints_left, joints_right,  is_train=False, num_proposals=args.num_proposals, sampling_timesteps=args.sampling_timesteps, num_joints=num_joints, num_joints_dense=num_joints_dense, dense_fuse_strategy=args.dense_fuse_strategy)
...
if args.resume or args.evaluate or args.finetune:
    if args.resume:
        ...
        model_name = args.evaluate
    else:
        model_name = args.finetune
    chk_filename = os.path.join(args.checkpoint, model_name)
...
test_generator = UnchunkedGenerator_Seq(None, out_poses_3d_test, out_poses_2d_test,
                                    ...
                                    kps_left=kps_left, kps_right=kps_right, joints_left=joints_left, joints_right=joints_right, valid_frame=total_valid_frame_test)
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        with torch.no_grad():
            ...
            if not args.no_eval:
                ...
                for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
                    ...
                    if epoch_loss_3d_train_eval is None:
                        epoch_loss_3d_train_eval = inputs_3d.shape[0] * inputs_3d.shape[1] * error.clone()
                    else:
                        epoch_loss_3d_train_eval += inputs_3d.shape[0] * inputs_3d.shape[1] * error.clone()
                ...
                for cam, batch, batch_2d, batch_valid, _ in test_generator.next_epoch():
                    ...
                    for batch_cnt in range(total_batch):
                        ...
                        error = mpjpe_diffusion_3dhp(predicted_3d_pos_single, inputs_3d_single, valid_frame_single.type(torch.bool), mean_pos=True)

                        valid_count = valid_frame_single.sum().item()

                        if iteration == 0:
                            epoch_loss_3d_valid = valid_count * error.clone()
                        else:
                            epoch_loss_3d_valid += valid_count * error.clone()
        ...
        if args.no_eval:
            ...
            print('[%d] time %.2f lr %f 3d_train %f 3d_pos_train %f 3d_pos_eval %f 3d_pos_valid %f' % (
                epoch + 1,
                elapsed,
                lr,
                losses_3d_train[-1],
                losses_3d_pos_train[-1],
                losses_3d_train_eval[-1][0],
                losses_3d_valid[-1][0]
            ))

            log_path = os.path.join(args.checkpoint, 'training_log.txt')
            f = open(log_path, mode='a')
            f.write('[%d] time %.2f lr %f 3d_train %f 3d_pos_train %f 3d_pos_eval %f 3d_pos_valid %f\n' % (
                epoch + 1,
                elapsed,
                lr,
                losses_3d_train[-1],
            ...
            if not args.nolog:
                writer.add_scalar("Loss/3d training eval loss", losses_3d_train_eval[-1][0] , epoch+1)
                writer.add_scalar("Loss/3d validation loss", losses_3d_valid[-1][0], epoch+1)
        if not args.nolog:
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        ...
        total_p_agg_seq = torch.zeros(args.sampling_timesteps).cuda()
        total_p_best_seq = torch.zeros(args.sampling_timesteps).cuda()
        total_j_agg_seq = torch.zeros(args.sampling_timesteps).cuda()
        total_j_best_seq = torch.zeros(args.sampling_timesteps).cuda()
        Num_Of_Seq = 6
        ...
        for _, batch, batch_2d, batch_valid, keys in test_generator.next_epoch():
            ...
            epoch_loss_3d_pos_j_best = torch.zeros(args.sampling_timesteps).cuda()
            epoch_loss_3d_pos_j_agg = torch.zeros(args.sampling_timesteps).cuda()
            N = 0
            # if keys != "TS5":
            #     continue
            ...
            for batch_cnt in range(total_batch):
                ...
                target_2d = torch.from_numpy(image_coordinates(inputs_2d_single[..., :2].cpu().numpy(), w=cam_data[0], h=cam_data[1]))[:, :, :17, :].cuda()
                ...
                valid_frame_single_indices = valid_frame_single.type(torch.bool).squeeze(2)
                error_j_best = torch.norm(joint_min_pose - inputs_3d_single.unsqueeze(1), dim=-1).transpose(1,2)[valid_frame_single_indices].transpose(0,1).reshape(t, -1).mean(dim=-1)
                error_j_agg = torch.norm(reproj_min_pose - inputs_3d_single.unsqueeze(1), dim=-1).transpose(1,2)[valid_frame_single_indices].transpose(0,1).reshape(t, -1).mean(dim=-1)

                valid_count = valid_frame_single_indices.sum().item()

                epoch_loss_3d_pos += valid_count * error.clone()
                epoch_loss_3d_pos_mean += valid_count * error_mean.clone()
                epoch_loss_3d_pos_j_best += valid_count * error_j_best.clone()
                epoch_loss_3d_pos_j_agg += valid_count * error_j_agg.clone()
            ...
            e1_j_best = (epoch_loss_3d_pos_j_best / N)
            e1_j_agg = (epoch_loss_3d_pos_j_agg / N)

            total_p_agg_seq += e1_mean
            total_p_best_seq += e1
            total_j_agg_seq += e1_j_agg
            total_j_best_seq += e1_j_best
            ...
            for ii in range(e1.shape[0]):
                ...
                print('step %d : Protocol #1 Error (MPJPE) J_Best:' % ii, e1_j_best[ii].item(), 'mm')
                f.write('step %d : Protocol #1 Error (MPJPE) J_Best: %f mm\n' % (ii, e1_j_best[ii].item()))
                print('step %d : Protocol #1 Error (MPJPE) J_Agg:' % ii, e1_j_agg[ii].item(), 'mm')
        ...
        f.write('----'+'Average'+'----\n')
        for ii in range(e1.shape[0]):
            print('step %d : Protocol #1 Error (MPJPE) P_Best:' % ii, total_p_best_seq[ii].item()/Num_Of_Seq, 'mm')
            f.write('step %d : Protocol #1 Error (MPJPE) P_Best: %f mm\n' % (ii,total_p_best_seq[ii].item()/Num_Of_Seq))
            print('step %d : Protocol #1 Error (MPJPE) P_Agg:' % ii, total_p_agg_seq[ii].item()/Num_Of_Seq, 'mm')
            f.write('step %d : Protocol #1 Error (MPJPE) P_Agg: %f mm\n' % (ii, total_p_agg_seq[ii].item()/Num_Of_Seq))
            print('step %d : Protocol #1 Error (MPJPE) J_Best:' % ii, total_j_best_seq[ii].item()/Num_Of_Seq, 'mm')
            f.write('step %d : Protocol #1 Error (MPJPE) J_Best: %f mm\n' % (ii, total_j_best_seq[ii].item()/Num_Of_Seq))
            print('step %d : Protocol #1 Error (MPJPE) J_Agg:' % ii, total_j_agg_seq[ii].item()/Num_Of_Seq, 'mm')
            f.write('step %d : Protocol #1 Error (MPJPE) J_Agg: %f mm\n' % (ii, total_j_agg_seq[ii].item()/Num_Of_Seq))
        print('----------')
        f.write('----------\n')


    return e1, e1_mean, e1_j_agg, e1_j_best

if args.render:
    print('Rendering is not supported for this dataset.')
    ...
    def run_evaluation_all_actions(actions, action_filter=None):
        ...
        gen = UnchunkedGenerator_Seq(None, out_poses_3d_test, out_poses_2d_test,
                                     ...
                                     joints_right=joints_right, valid_frame=total_valid_frame_test)
        #e1, e2, e3, ev = evaluate(gen)
        e1, e1_mean, e1_j_agg, e1_j_best = evaluate(gen)

        # joints_errs_list.append(joints_errs)

        errors_p1.append(e1)
        errors_p1_mean.append(e1_mean)
        errors_j_agg.append(e1_j_agg)
        errors_j_best.append(e1_j_best)
