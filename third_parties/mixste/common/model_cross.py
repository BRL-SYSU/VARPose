...
class  MixSTE2_ConcatDense(nn.Module):
    def __init__(self, num_frame=9, num_joints=161, out_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
                 num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,  norm_layer=None, remove_pseudo=False):
        """    ##########hybrid_backbone=None, representation_size=None,
        Args:
            num_frame (int, tuple): input frame number
            num_joints (int, tuple): joints number
        ...
        self.remove_pseudo = remove_pseudo
        if remove_pseudo:
            # Mask Pseudo Joints
            ...
            self.valid_joints = [i for i in range(num_joints) if i not in self.pseudo_joints_idx]
            self.actual_num_joints = len(self.valid_joints)
        else:
            self.valid_joints = [i for i in range(num_joints)]
            self.actual_num_joints = num_joints

        ### spatial patch embedding
        self.Spatial_patch_to_embedding = nn.Linear(in_chans, embed_dim_ratio)
        # self.Spatial_patch_to_embedding = nn.Conv1d(in_chans, embed_dim_ratio, kernel_size=1, stride=1)
        self.Spatial_pos_embed = nn.Parameter(torch.zeros(1, self.actual_num_joints, embed_dim_ratio))
        # nn.init.kaiming_normal_(self.Spatial_pos_embed)
        # torch.nn.init.normal_(self.Spatial_pos_embed, std = .02)
        ...
        ####### A easy way to implement weighted mean
        # self.weighted_mean = torch.nn.Conv1d(in_channels=num_frame, out_channels=num_frame, kernel_size=1)
        if not remove_pseudo:
            self.head_joints = nn.Sequential(
                nn.LayerNorm(num_joints),
                nn.Linear(num_joints, out_joints),
            )
    ...
    def forward(self, x):
        ...
        if not self.remove_pseudo:
            x = self.head_joints(x.transpose(-1,-2)).transpose(-1,-2).contiguous()
        # st = time.time()
        x = self.head(x)
        # et = time.time()
        # print('head  ', (et-st)*2000)
        ...
        if self.remove_pseudo:
            x = x.view(b, f, n, 3)
            x = x[:, :, :self.out_joints, :] 
        else:
            x = x.view(b, f, self.out_joints, -1)
