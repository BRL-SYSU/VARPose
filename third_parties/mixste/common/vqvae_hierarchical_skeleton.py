...
from common.quant import VectorQuantizerForSkeleton
from common.layers import MlpMixerBlock, ResiLinear, TokenExpansion, TokenAggregation
...
class HierarchicalEncoder(nn.Module):
    def __init__(self,
        ...
        self.token_expansion = TokenExpansion(tokens_per_joint=tokens_per_joint)
...
class HierarchicalDecoder(nn.Module):
    def __init__(self,
        ...
        self.token_aggregation = TokenAggregation(tokens_per_joint=tokens_per_joint)
...
class HierarchicalVQVAE(VQVAEBase):
    def __init__(
        ...
        v_patch_nums=[48, 102, 192, 288, 432, 576], # number of patches for each scale, h_{1 to K} = w_{1 to K} = v_patch_nums[k]
        ...
        self.current_level = -1  # -1 trains all levels together; 0, 1, 2 train the corresponding level.
    ...
    def forward(self, p_BJDs: list[torch.Tensor], hasLoss=True):
        ...
        for i, p_BJD in enumerate(p_BJDs):
            ...
            if self.current_level < 0 or i<=self.current_level:
                ...
                # Keep GPU memory usage stable.
                phat = self.decoder[i](embeddings)

        if hasLoss:
            # Progressive training: only compute reconstruction loss for the current level.
            if self.current_level >= 0:
    ...
    def set_progressive_level(self, level):
        ...
        Args:
            level (int): Level index. -1 trains all levels together;
                0, 1, and 2 train the corresponding level.
        """
