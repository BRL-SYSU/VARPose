...
def parse_args():
    ...
    # Custom data paths (optional, overrides default paths)
    parser.add_argument('--data-2d-path', type=str, default='', metavar='PATH',
                        help='custom path to 2D keypoints .npz file (overrides default)')
    parser.add_argument('--data-3d-path', type=str, default='', metavar='PATH',
                        help='custom path to 3D poses .npz file (overrides default)')
    ...
    parser.add_argument('--split_epoch', default=[0.5, 0.25, 0.25], type=int, nargs='+', help='split epoch to different train task and sum up to 1')
    parser.add_argument('-b', '--batch-size', default=405, type=int, metavar='N', help='batch size in terms of predicted frames')
    parser.add_argument('-tb', '--test_batch_size', default=2, type=int, metavar='N', help='batch size in terms of predicted frames')
    ...
    parser.add_argument('--only_17', action='store_true', help='use only 17 joints')
    parser.add_argument('--num-joints', type=int ,default=161, help='number of joints')
    parser.add_argument("--mixste_path_to_refine", default='checkpoints/GT_17__20251017T07-50-50/best_epoch.bin', type=str, metavar='PATH', help='path to the model to refine')
    parser.add_argument("--hierarchical_model_path", type=str, metavar='PATH', help='path to the hierarchical model')
    ...
    parser.add_argument('--use_h36m_form', action='store_true', help='use h36m form')
    ...
    parser.add_argument('--viz-frame-idx', type=int, default=-1,  metavar='N', help='render only one frame')
    parser.add_argument('--viz-limit', type=int, default=-1, metavar='N', help='only render first N frames')
    parser.add_argument('--viz-downsample', type=int, default=1, metavar='N', help='downsample FPS by a factor N')
    parser.add_argument('--viz-size', type=int, default=5, metavar='N', help='image size')
    parser.add_argument('--viz-single-frame-path', type=str, default=None, metavar='PATH', help='path to single frame image for visualization')
    ...
    # 3DPW evaluation
    parser.add_argument('--eval-3dpw', action='store_true', help='enable 3DPW evaluation mode')
    parser.add_argument('--data-path-3dpw', type=str, default='/data/3dpw/processedByPuky/h36m_for_lifting_darkpose_dense_conf.npz', metavar='PATH', help='path to 3DPW dataset')
    parser.add_argument('--norm-scale', type=float, default=1.85, metavar='SCALE', help='scale factor for 3DPW data normalization (only effective in eval-3dpw mode)')
    parser.add_argument('--generalization-mode', type=str, default='bbox', choices=['bbox', 'camera'],
                        help='generalization mode for 3DPW: bbox (default) or camera projection')
    
    # Save predictions
    parser.add_argument('--save-prediction', type=str, default="", metavar='PATH', 
                        help='path to save predictions in npz format (no evaluation)')
