...
from common.vqvae_hierarchical_skeleton import HierarchicalVQVAE
...
# Use custom 3D data path if provided, otherwise use default
if args.data_3d_path:
    dataset_path = args.data_3d_path
    print(f'Using custom 3D data path: {dataset_path}')
else:
    dataset_path = 'data/data_3d_' + args.dataset + '.npz'
    if args.dataset == 'h36m':
        dataset_path = 'data/data_3d_h36m_remapped.npz'
...
# Use custom 2D data path if provided, otherwise use default
if args.data_2d_path:
    keypoints_path = args.data_2d_path
    print(f'Using custom 2D data path: {keypoints_path}')
else:
...
kps_dense_left = list(symmetry_dense[:, 0])
kps_dense_right = list(symmetry_dense[:, 1])
pseudo_joints_idx_48 = np.array([23, 35, 39])+17
pseudo_joints_idx_96 = np.array([3, 7, 11, 15, 19, 23, 27, 46, 47, 70, 71, 78, 79])+17+48
pseudo_joints_idx = np.concatenate([pseudo_joints_idx_48, pseudo_joints_idx_96], axis=0)
###################
for subject in dataset.subjects():
...
if not args.only_17 and (args.keypoints in ['gt_contiguous_161j', 'cpn_contiguous_161j', 'gt_inter_trash', 'cpn_inter_trash']):
    print(f"Initializing Multi-scale Model ({num_joints} -> 17 joints)")
    print(f"num_joints: {num_joints}")
    kps_left += kps_dense_left
    kps_right += kps_dense_right

    # hierarchical_model = HierarchicalVQVAE()
    # hierarchical_model.load_state_dict(torch.load(args.hierarchical_model_path), strict=False)

    # model_pos_train = DenseSTE_With_Codebook(
    #     embed_dim=args.cs,
    #     num_frames=receptive_field,
    #     depth=args.dep,
    #     vqvae_hierarchical=hierarchical_model,
    # )

    # model_pos = DenseSTE_With_Codebook(
    #     embed_dim=args.cs,
    #     num_frames=receptive_field,
    #     depth=args.dep,
    #     vqvae_hierarchical=hierarchical_model,
    ...
    model_pos_train =  MixSTE2_ConcatDense(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
            num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0.1)
    model_pos =  MixSTE2_ConcatDense(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
            num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0)
else:
    num_joints = 17
    print(f"Initializing Standard MixSTE2 Model ({num_joints} joints)")
...
# if not args.only_17:
#     checkpoint = torch.load(args.mixste_path_to_refine, map_location=lambda storage, loc: storage, weights_only=False)
#     print('This mixste model was trained for {} epochs'.format(checkpoint['epoch']))

#     new_state_dict = OrderedDict()
#     for k, v in checkpoint['model_pos'].items():
#         name = k[7:] if k.startswith('module.') else k
#         new_state_dict[name] = v
#     model_pos_train.module.backbone.load_state_dict(new_state_dict, strict=False)
#     model_pos_train.module.backbone.cuda()
#     model_pos_train.module.backbone.eval()
#     model_pos.module.backbone.load_state_dict(new_state_dict, strict=False)
#     model_pos.module.backbone.cuda()
#     model_pos.module.backbone.eval()
...
def eval_data_prepare(receptive_field, inputs_2d, inputs_3d):
    ...
    assert inputs_2d.shape[:2] == inputs_3d.shape[:2], \
        f"2d and 3d inputs batch and frame dimensions must be same! Got {inputs_2d.shape} and {inputs_3d.shape}"
    inputs_2d_p = torch.squeeze(inputs_2d)
...
if not args.evaluate:
    ...
    train_augment = args.data_augmentation
    ...
    train_generator = ChunkedGenerator_Seq(args.batch_size//args.stride, cameras_train, poses_train, poses_train_2d, args.number_of_frames,
                                       pad=pad, causal_shift=causal_shift, shuffle=True, augment=train_augment,
                                       kps_left=kps_left, kps_right=kps_right, joints_left=joints_left, joints_right=joints_right)
    ...
    while epoch < args.epochs:
        ...
        with torch.no_grad():
            ...
            if not args.no_eval:
                ...
                for _, cam, batch, batch_2d in test_generator.next_epoch():
                    ...
                    cam = cam.expand(inputs_3d.shape[0], -1)
                    ...
                    # set root as zero
                    predicted_3d_pos -= predicted_3d_pos[:, :, 0:1]
                    loss_3d_pos = mpjpe(predicted_3d_pos, inputs_3d)
                ...
                N = 0
                for _, cam, batch, batch_2d in train_generator_eval.next_epoch():
...
# Evaluate
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None, is_augmented=False, collect_predictions=False):
    epoch_loss_3d_pos = 0
    ...
    with torch.no_grad():
        ...
        # else:
            # model_traj.eval()
        N = 0
        for meta, cam, batch, batch_2d in test_generator.next_epoch():
            inputs_2d = torch.from_numpy(batch_2d.astype('float32'))
            inputs_3d = torch.from_numpy(batch.astype('float32'))
            if args.only_17:
                inputs_2d = inputs_2d[:, :, :17, :]
            ...
            if collect_predictions:
                ...
                predictions_dict[key].append(predicted_3d_pos.squeeze().cpu().numpy())
                continue
    ...
    if collect_predictions:
        # Stack predictions for each key and skip metric computation in export mode.
        for key in predictions_dict:
            predictions_dict[key] = np.concatenate(predictions_dict[key], axis=0)
...
def reshape_prediction(predictions, receptive_field, original_length):
    """
    Reshape the predictions to match the original length of the input sequence.
    """
    if original_length / receptive_field > original_length // receptive_field:
        batch_num = (original_length // receptive_field) + 1
        prediction_reshaped = np.empty((original_length, predictions.shape[-2], predictions.shape[-1]))
        for i in range(batch_num - 1):
            prediction_reshaped[i*receptive_field:(i+1)*receptive_field] = predictions[i]
        left_frames = original_length - (batch_num - 1) * receptive_field
        prediction_reshaped[-left_frames:] = predictions[-1, -left_frames:]
        return prediction_reshaped
    else:
        return predictions.reshape(original_length, predictions.shape[1], predictions.shape[2])


# Prepare all_actions and fetch_actions for both save-prediction and evaluation modes
all_actions = {}
...
def fetch_actions(actions):
    ...
    for subject, action in actions:
        ...
        for i in range(len(poses_2d)): # Iterate across cameras
            out_poses_2d.append(poses_2d[i])
            out_metadata.append((subject, action, i))  # (subject_id, action_name, camera_idx)
...
if args.render:
    ...
    cams = dataset.cameras()[args.viz_subject]
    cam = [cams[args.viz_camera]['intrinsic'],]
    ground_truth = None
    ...
    gen = UnchunkedGenerator_Seq(cam, [ground_truth], [input_keypoints],
    ...
    if args.viz_output is not None:
        ...
        render_animation(input_keypoints, keypoints_metadata, anim_output,
                        ...
                        input_video_skip=args.viz_skip, frame_dir="checkpoints/h36m_per_action", save_frames=True, 
                        frame_idx=args.viz_frame_idx, single_frame_image_path=args.viz_single_frame_path)

# Save predictions mode
eval_augment = args.test_time_augmentation

if args.save_prediction != '':
    print('Saving predictions to', args.save_prediction)
    predictions_dict = {}
    
    if args.eval_3dpw:
        # 3DPW
        print('Loading 3DPW dataset from:', args.data_path_3dpw)
        threedpw_data = np.load(args.data_path_3dpw, allow_pickle=True)
        data_dict = threedpw_data['data'].item()
        
        cameras_all, poses_all, poses_2d_all, metadata = fetch_3dpw_data(data_dict, action=None, scale=args.norm_scale, generalization_mode=args.generalization_mode)
        
        gen = UnchunkedGenerator_Seq(cameras_all, poses_all, poses_2d_all,
                                   metadata=metadata, pad=pad, causal_shift=causal_shift,
                                   augment=eval_augment, kps_left=kps_left, kps_right=kps_right,
                                   joints_left=joints_left, joints_right=joints_right)
        
        raw_predictions = evaluate(gen, collect_predictions=True)
        
        # reshape
        for meta, pred_list in raw_predictions.items():
            original_length = data_dict[meta]['poses_3d'].shape[0]
            pred = reshape_prediction(pred_list, receptive_field, original_length)
            predictions_dict[meta] = pred
    else:
        # H36M
        cameras_all = []
        poses_all = []
        ...
        for action_key in all_actions.keys():
            cameras_act, poses_act, poses_2d_act, metadata = fetch_actions(all_actions[action_key])
            cameras_all.extend(cameras_act)
            poses_all.extend(poses_act)
            poses_2d_all.extend(poses_2d_act)
            metadata_all.extend(metadata)
        
        gen = UnchunkedGenerator_Seq(cameras_all, poses_all, poses_2d_all,
                                   metadata=metadata_all, pad=pad, causal_shift=causal_shift,
                                   augment=eval_augment, kps_left=kps_left, kps_right=kps_right,
                                   joints_left=joints_left, joints_right=joints_right)
        ...
        # reshape
        for meta, pred_list in raw_predictions.items():
            subject, action, cam_idx = meta
            original_length = dataset[subject][action]['positions_3d'][cam_idx].shape[0]
            pred = reshape_prediction(pred_list, receptive_field, original_length)
            predictions_dict[meta] = pred

    np.savez(args.save_prediction, predictions_dict=predictions_dict)
    print(f'Predictions saved to {args.save_prediction}')
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        # 3DPW evaluation mode
        if args.eval_3dpw:
            print('Loading 3DPW dataset from:', args.data_path_3dpw)
            threedpw_data = np.load(args.data_path_3dpw, allow_pickle=True)
            data_dict = threedpw_data['data'].item()
            
            # Get all unique action names from data keys
            ...
            if action_filter is not None:
                filtered_actions = []
                for a in action_filter:
                    filtered_actions.extend([name for name in action_names if name.startswith(a)])
                action_names = filtered_actions
            
            print('Evaluating on 3DPW actions:', action_names)
            
            # Evaluate each action
            for action_name in sorted(action_names):
                cameras_act, poses_act, poses_2d_act, metadata = fetch_3dpw_data(data_dict, action_name, scale=args.norm_scale, generalization_mode=args.generalization_mode)
                
                if len(cameras_act) == 0:
                    continue
                
                gen = UnchunkedGenerator_Seq(cameras_act, poses_act, poses_2d_act,
                                         metadata=metadata, pad=pad, causal_shift=causal_shift, augment=eval_augment,
                                         kps_left=kps_left, kps_right=kps_right, joints_left=joints_left,
                                         joints_right=joints_right)
                e1, e2, e3, ev = evaluate(gen, action_name, is_augmented=eval_augment)
            ...
            for action_key in actions.keys():
                ...
                cameras_act, poses_act, poses_2d_act, metadata = fetch_actions(actions[action_key])
                gen = UnchunkedGenerator_Seq(cameras_act, poses_act, poses_2d_act,
                                         metadata=metadata, pad=pad, causal_shift=causal_shift, augment=eval_augment,
                                         kps_left=kps_left, kps_right=kps_right, joints_left=joints_left,
                                         joints_right=joints_right)
                e1, e2, e3, ev = evaluate(gen, action_key, is_augmented=eval_augment)
